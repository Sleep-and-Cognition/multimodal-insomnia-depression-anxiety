...
% http://brainder.org
%
% Edited by Siemon de Lange, April 14 2023, to not include NaNs.
...
if nargout == 2 || nargout == 3,
    ...
    [~,oidxR] = sort(oidx);
    pcorfull = nan(size(indx));
    pcorfull(indx) = pcor(oidxR);
    varargout{2} = pcorfull;
end
...
if nargout == 3,
    ...
    padjfull = nan(size(indx));
    padjfull(indx) = padj(oidxR);
    varargout{3} = padjfull;
